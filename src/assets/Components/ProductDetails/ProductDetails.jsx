...
const ProductDetails = ({ selectedProductdetail, removeSeletecdProduct }) => {
  return (
    <>
      ...
      <section className='px-2 max-xl:flex-col sm:px-15 lg:px-24 xl:px-40 border flex  gap-5 items-start py-5'>
        <img src={selectedProductdetail.image} alt={selectedProductdetail.name} className='rounded-2xl w-[500px] max-xl:w-full' />

        <div className='border p-5 flex flex-col items-start'>
          ...
          <div className='flex items-center gap-3'><span className='bg-green-600 rounded-sm text-amber-50 px-1.5 my-2 py-0 text-sm flex items-center gap-1'><Star width={17} /> {selectedProductdetail.rating}</span> <span className='text-gray-500 text-sm'>86 ratings & 105 reviews</span></div>
          ...
          <div className='sm:gap-5 gap-3 my-3 flex  max-sm:flex-col'>
            <button className='bg-purple-500 p-4 text-white font-semibold cursor-pointer hover:shadow hover:shadow-purple-300 transition-all rounded-md inline-flex '><ShoppingCart width={20}/> &nbsp; ADD TO CART</button>
